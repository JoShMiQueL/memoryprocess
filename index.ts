...
export function findPattern(handle: number, pattern: string, flags: number, patternOffset: number): number;
...
export function findPattern(handle: number, pattern: string, flags: number, patternOffset: number, callback?: FindPatternCallback): number | void {
  if (!callback) {
