...
import { STRUCTRON_TYPE_STRING, isDataTypeBE } from "./src/utils";
...
export function closeProcess(handle: number): undefined {
  return memoryjs.closeProcess(handle);
}

export function getProcesses(): ProcessInfo[];
export function getProcesses(callback: GetProcessesCallback): undefined;
...
export function readMemory<T extends DataType>(
  handle: number,
...
export function readMemory<T extends DataType>(
  ...
  if (isDataTypeBE(dataType)) {
    return readMemoryBE(handle, address, dataType, callback as ReadMemoryBECallback<DataTypeToType<T>>);
  }
...
export function readMemoryBE<T extends DataType>(
  handle: number,
  address: number | bigint,
  dataType: T,
): DataTypeToType<T>;
...
export function readMemoryBE<T extends DataType>(
  ...
  if (!callback) {
    ...
    return value as DataTypeToType<T>;
  }

  callback(
    value === null ? "Invalid data type argument!" : "",
    value as DataTypeToType<T>,
  );
}
...
export function writeMemory<T extends DataType>(
  handle: number,
  address: number | bigint,
  value: DataTypeToWriteType<T>,
  dataType: T,
): undefined {
  let dataValue: DataTypeToWriteType<T> = value;
  if (
    (dataType === "str" || dataType === "string") &&
    typeof value === "string"
  ) {
    dataValue = `${value}\0` as DataTypeToWriteType<T>; // add terminator
  }
...
export function writeMemoryBE<T extends DataType>(
  handle: number,
  address: number | bigint,
  value: DataTypeToWriteType<T>,
  dataType: T,
...
export function virtualAllocEx(
  ...
  if (!callback) {
    return memoryjs.virtualAllocEx(
      handle,
      address,
      size,
      allocationType,
...
export function getRegions(handle: number, callback: GetRegionsCallback): undefined;
export function getRegions(
  handle: number,
  callback?: GetRegionsCallback,
...
export function unloadDll(
  handle: number,
  moduleAddressOrName: number | bigint | string,
  callback?: UnloadDllCallback,
): boolean | undefined {
  // Handle synchronous calls
  if (!callback) {
